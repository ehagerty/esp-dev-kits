...
#include "brookesia/system_core/base/context.hpp"
#include "brookesia/system_core/base/app.hpp"
...
bool App::notifyCoreClosed(void) const
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(core_utils_check_event_code_valid(event_code), false, "Event code is invalid");
...
bool App::startRecordResource(void)
{
    ...
    if (_active_config.flags.enable_resize_visual_area) {
        ESP_UTILS_LOGD(
            "Resieze screen to visual area[(%d,%d)-(%d,%d)]",
            static_cast<int>(visual_area.x1), static_cast<int>(visual_area.y1),
            static_cast<int>(visual_area.x2), static_cast<int>(visual_area.y2)
        );
...
bool App::setVisualArea(const lv_area_t &area)
{
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UTILS_LOGD(
        "App(%s: %d) set origin visual area[(%d,%d)-(%d,%d)]", getName(), _id,
        static_cast<int>(area.x1), static_cast<int>(area.y1), static_cast<int>(area.x2), static_cast<int>(area.y2)
    );
...
bool App::calibrateVisualArea(void)
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UTILS_LOGD(
        "App(%s: %d) calibrate visual area[origin: (%d,%d)-(%d,%d)]", getName(), _id,
        static_cast<int>(visual_area.x1), static_cast<int>(visual_area.y1),
        static_cast<int>(visual_area.x2), static_cast<int>(visual_area.y2)
    );
    ...
    ESP_UTILS_LOGD(
        "Calibrate visual area[(%d,%d)-(%d,%d)]", static_cast<int>(visual_area.x1), static_cast<int>(visual_area.y1),
        static_cast<int>(visual_area.x2), static_cast<int>(visual_area.y2)
    );
