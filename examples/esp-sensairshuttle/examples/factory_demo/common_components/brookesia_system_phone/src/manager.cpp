...
#include "brookesia/system_phone/manager.hpp"
...
bool Manager::begin(void)
{
    ...
    if (data.flags.enable_gesture) {
        ...
        if (touch == nullptr) {
            ...
            touch = core_utils_get_input_dev(_system_context.getDisplayDevice(), LV_INDEV_TYPE_POINTER);
...
bool Manager::processStatusBarScreenChange(Screen screen, void *param)
{
    ...
    ESP_UTILS_LOGD("Visual Mode: status bar(%d)", static_cast<int>(status_bar_visual_mode));
...
bool Manager::processNavigationBarScreenChange(Screen screen, void *param)
{
    ...
    ESP_UTILS_LOGD("Visual Mode: navigation bar(%d)", static_cast<int>(navigation_bar_visual_mode));
...
bool Manager::processNavigationEvent(base::Manager::NavigateType type)
{
    ...
    ESP_UTILS_LOGD("Process navigation event type(%d)", static_cast<int>(type));
...
void Manager::onRecentsScreenGesturePressEventCallback(lv_event_t *event)
{
    ...
    ESP_UTILS_LOGD("Recents screen press(%d, %d)", static_cast<int>(start_point.x), static_cast<int>(start_point.y));
}
...
void Manager::onRecentsScreenGestureReleaseEventCallback(lv_event_t *event)
{
    ...
    ESP_UTILS_CHECK_FALSE_EXIT(core_utils_check_event_code_valid(event_code), "Invalid event code");
