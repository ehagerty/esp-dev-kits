...
#include "brookesia/system_phone/widgets/gesture.hpp"
...
bool Gesture::begin(lv_obj_t *parent)
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(core_utils_check_event_code_valid(press_event_code), false,
                                 "Invalid press event code");
    pressing_event_code = core.getFreeEventCode();
    ESP_UTILS_CHECK_FALSE_RETURN(core_utils_check_event_code_valid(pressing_event_code), false,
                                 "Invalid pressing event code");
    release_event_code = core.getFreeEventCode();
    ESP_UTILS_CHECK_FALSE_RETURN(core_utils_check_event_code_valid(release_event_code), false,
...
{
    ESP_UTILS_LOGD("Set indicator bar(%d) length(%d)", static_cast<int>(type), length);
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    auto type_int = static_cast<int>(type);
    ESP_UTILS_CHECK_VALUE_RETURN(type_int, 0, static_cast<int>(Gesture::IndicatorBarType::MAX), false, "Invalid indicator bar type");
...
{
    ESP_UTILS_LOGD("Set indicator bar(%d) length by offset(%d)", static_cast<int>(type), offset);
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    auto type_int = static_cast<int>(type);
    ESP_UTILS_CHECK_VALUE_RETURN(type_int, 0, static_cast<int>(Gesture::IndicatorBarType::MAX), false, "Invalid indicator bar type");
...
{
    ESP_UTILS_LOGD("Set indicator bar(%d) visible(%d)", static_cast<int>(type), visible);
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    auto type_int = static_cast<int>(type);
    ESP_UTILS_CHECK_VALUE_RETURN(type_int, 0, static_cast<int>(Gesture::IndicatorBarType::MAX), false, "Invalid indicator bar type");
...
bool Gesture::controlIndicatorBarScaleBackAnim(Gesture::IndicatorBarType type, bool start)
{
    ...
    ESP_UTILS_LOGD("Control indicator bar(%d) scale back animation(%d)", static_cast<int>(type), start);
    ESP_UTILS_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    auto type_int = static_cast<int>(type);
    ESP_UTILS_CHECK_VALUE_RETURN(type_int, 0, static_cast<int>(Gesture::IndicatorBarType::MAX), false, "Invalid indicator bar type");
...
bool Gesture::updateByNewData(void)
{
    ...
    for (int i = 0; i < static_cast<int>(Gesture::IndicatorBarType::MAX); i++) {
        ...
        lv_anim_set_path_cb(_indicator_bar_scale_back_anims[i].get(),
                            core_utils_get_anim_path_cb(bar_data.animation.scale_back_path_type));
