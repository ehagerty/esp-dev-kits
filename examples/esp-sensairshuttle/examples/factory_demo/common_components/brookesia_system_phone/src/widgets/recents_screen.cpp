...
#include "brookesia/system_phone/widgets/recents_screen.hpp"

#define MEMORY_LABEL_TEXT_FORMAT        "%d%% SRAM | %d%% PSRAM used"
...
bool RecentsScreen::begin(lv_obj_t *parent)
{
    ...
    lv_label_set_text_fmt(_memory_label.get(), MEMORY_LABEL_TEXT_FORMAT, 0, 0);
...
{
    int internal_used_percent = 0;
    int external_used_percent = 0;

    ESP_UTILS_LOGD("Set memory label");
    ESP_UTILS_CHECK_FALSE_RETURN(_memory_label != nullptr, false, "Memory label is disabled");

    // Calculate used percentage for internal SRAM (HP SRAM)
    // used = total - free, used_percent = (used * 100) / total
    if (internal_total > 0) {
        int internal_used = internal_total - internal_free;
        internal_used_percent = (internal_used * 100) / internal_total;
    }

    // Calculate used percentage for external PSRAM
    if (external_total > 0) {
        int external_used = external_total - external_free;
        external_used_percent = (external_used * 100) / external_total;
    }

    lv_label_set_text_fmt(_memory_label.get(), MEMORY_LABEL_TEXT_FORMAT, internal_used_percent, external_used_percent);
...
bool RecentsScreen::updateByNewData(void)
{
    ...
    if (_data.flags.enable_memory) {
        ...
        // Add extra left offset to move text left (increase the negative offset value)
        lv_obj_align(_memory_label.get(), LV_ALIGN_RIGHT_MID, -_data.memory.main_layout_x_right_offset - 30, 0);
