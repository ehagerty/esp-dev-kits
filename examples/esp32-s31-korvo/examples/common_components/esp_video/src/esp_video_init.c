...
typedef struct video_device_init_config {
    ...
    void *clk_priv;
    bool init_clk_before_sccb;
    uint32_t clk_stabilize_delay_ms;
...
#define ESP_VIDEO_DVP_XCLK_STABILIZE_DELAY_MS    50
...
static esp_err_t esp_video_init_sensor_and_video_device(const video_device_init_config_t *config)
{
    ...
    /* Some DVP sensors need XCLK running before the first SCCB register read. */
    if (config->init_clk_before_sccb && config->init_clk_func) {
        ESP_GOTO_ON_ERROR(config->init_clk_func(config->clk_priv), fail_0, TAG, "Failed to initialize clock");
        clk_initialized = true;
        if (config->clk_stabilize_delay_ms > 0) {
            vTaskDelay(pdMS_TO_TICKS(config->clk_stabilize_delay_ms));
        }
    }
    ...
    ESP_GOTO_ON_ERROR(sccb_new_i2c_io(i2c_handle, &sccb_i2c_config, &sccb_io), fail_clk, TAG, "Failed to initialize SCCB device");

    if (!clk_initialized && config->init_clk_func) {
...
fail_clk:
    if (clk_initialized && config->deinit_clk_func) {
        ...
        if (flags & ESP_VIDEO_INIT_FLAGS_DVP) {
            if (!(s_video_device_inited_flags & ESP_VIDEO_INIT_FLAGS_DVP) && p->port == ESP_CAM_SENSOR_DVP && config->dvp != NULL) {
                ...
                video_device_init_config_t dvp_init_config = {
                    ...
                    .clk_priv = (void *)dvp,
                    .init_clk_before_sccb = true,
                    .clk_stabilize_delay_ms = ESP_VIDEO_DVP_XCLK_STABILIZE_DELAY_MS,
                    .device_inited = &device_inited,
                };
